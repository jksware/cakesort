...
namespace BBox
{
	namespace CUDA
	{
		__global__ void binSearchTidGroupKernel(const int *start, int *subset, const int length,
			const int paddedLength, const int globalWS);

		__global__ void setGidKernel(int* data);			

		template <typename T>
		__global__ void bitonicSortKernel(T *key, int *index, const int stage, int pass)
		{
		...
		__global__ void bitonicSortLayeredKernel(Data *data, const int stage, int pass,
			 const int *subset, const int *start, const int *sortedStart,
			 const int *permutation, const int assumeSorted, const int globalWS)
		{
		...
		__global__ void bitonicSortForSorted(Data *data, const int stage, const int pass, const int globalWS)
		{
			int localId = blockDim.x * blockIdx.x + threadIdx.x;
			if (localId >= globalWS)
				return;
		...
		template <typename Data>
		__global__ void swapKernel(Data *data, const int from, int width, const int to)
		{
		...
		template <typename Data, typename Key>
		__global__ void bitonicSortIndexLayeredKernel(const Data *data, const int stage, 
			const int pass, const int *subset, const int *start, const int *sortedStart, 
			const int *permutation, const int assumeSorted, const int globalWS, int *index)
		{
			int tid = blockDim.x * blockIdx.x + threadIdx.x;
			if (tid >= globalWS)
				return;
		...
		template <typename Data>
		__global__ void mergeSortKernel(Data *data, int *index)
		{
		...
		template <typename Data, typename Key>
		__global__ void mergeSortLayeredKernel(Data *data, const int *start, const int *permutation, const int assumeSorted,
		...
		template <typename T>
		class CakeSort : public CudaBlackBox<SegmentedArray<T>, SegmentedArray<T>>
		{
			typedef T Data;
			typedef T Key;		
		...
		public:
			...
			CakeSort(bool assumeSorted, bool hybrid, bool useHost, bool reuseStartBuffer, bool useIndex)
				: assumeSorted(assumeSorted), hybrid(hybrid), useHost(useHost), reuseStartBuffer(reuseStartBuffer), 
					useIndex(useIndex)
			{
			}

			std::string Name() override { return "Cake Sort CUDA"; }
			std::string ShortName() override { return "cuCakeSort"; }
			...
			void BinarySearch(int inputLength, int outputLength, thrust::device_vector<int> const& input, thrust::device_vector<int> & output)
			{
				int globalWorkSize = outputLength;
				int localWorkSize = this->BlockSize;
				int npotLength = NextGreatestPowerOfTwo(inputLength);

				binSearchTidGroupKernel<<<static_cast<int>(ceil(static_cast<float>(globalWorkSize)/localWorkSize)), localWorkSize, sizeof(T) * localWorkSize>>>(
			...
			bool Process() override
			{
				...
				if (!assumeSorted) {
					...
					if (useHost) {
						std::vector<std::pair<int, int>> _pairs{ static_cast<unsigned int>(_segments)};
						...
						setGidKernel<<<static_cast<int>(ceil(static_cast<float>(globalWorkSize)/this->BlockSize)), this->BlockSize>>>(
							thrust::raw_pointer_cast(_permutationBuffer.data()));

						int beginPartitionsStageNumber = 0;
						if (hybrid) {
							mergeSortKernel<<<static_cast<int>(ceil(static_cast<float>(globalWorkSize)/this->BlockSize)), 
								this->BlockSize, 2 * sizeof(T) * this->BlockSize>>>(
						...
						for (int stage = beginPartitionsStageNumber; stage < npotPartitionStagesNumber; stage++) {
							for (int pass = 0; pass < stage + 1; ++pass) {
								bitonicSortKernel<<<static_cast<int>(ceil(static_cast<float>(globalWorkSize)/this->BlockSize)), this->BlockSize>>>(
									thrust::raw_pointer_cast(_rsCountsBuffer.data()),
				...
				for (int stage = 0; stage < endStage; ++stage) {
					unsigned int lowIndex = _rsCounts.rend() - std::upper_bound(_rsCounts.rbegin(), _rsCounts.rend(), 1 << stage);
				...
				if (hybrid && !useIndex && _rsCounts[0] >= hybridMinimumSize) {
					for (int count = 0; ; ) {
						...
						mergeSortLayeredKernel<T, T><<<static_cast<int>(ceil(globalMergeWS/localMergeWS)), 
				...
				if (!useIndex) {
					if (!assumeSorted) {
						for (int stage = beginStage; stage < endStage; ++stage) {
							...
							for (int pass = 0; pass < stage + 1; ++pass) {
								bitonicSortLayeredKernel<<<static_cast<int>(ceil(static_cast<float>(globalWorkSize) / this->BlockSize)), this->BlockSize >>> (
				...
				if (assumeSorted) {
					for (int width = 1 << endStage;; width >>= 1) {
						...
						swapKernel<<<static_cast<int>(ceil(globalWs/this->BlockSize)), this->BlockSize>>>(
